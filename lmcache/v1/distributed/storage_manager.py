...
class StorageManager:
    def __init__(self, config: StorageManagerConfig):
        ...
        # Retention budget: a fraction of total L2 capacity, validated
        # against each adapter's watermark so eviction can always find
        # victims. Capacity is taken at boot; adapters added at runtime
        # do not grow the budget.
        l2_capacity_bytes = 0
        for adapter_id, ac in zip(
            ...
            if ac.eviction_config is None:
                continue
            if (
                config.retention_max_fraction > 0
                and config.retention_max_fraction
                >= ac.eviction_config.trigger_watermark
            ):
                raise ValueError(
