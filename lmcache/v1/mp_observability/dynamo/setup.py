...
def setup_dynamo_kv_publishing(
    ...
    from lmcache.v1.mp_observability.dynamo.queue_emitter import QueueKvEventEmitter
    ...
    if obs_config.dynamo_kv_sink == "connector":
        emitter = QueueKvEventEmitter(medium=obs_config.dynamo_medium)
        ctx.dynamo_kv_event_queue = emitter
    else:
    ...
    publisher = DynamoKvPublisher(
        ...
        ctx.chunk_size,
        hash_block_size=obs_config.dynamo_hash_block_size,
    )
