...
class MPCacheServerContext:
    ...
    def __init__(
        ...
        # Set by setup_dynamo_kv_publishing when --dynamo-kv-sink=connector.
        self._dynamo_kv_event_queue = None
    ...
    @property
    def dynamo_kv_event_queue(self):
        """Connector-sink event queue, or ``None`` when the sink is ZMQ/off."""
        return self._dynamo_kv_event_queue

    @dynamo_kv_event_queue.setter
    def dynamo_kv_event_queue(self, queue) -> None:
        """Install the connector-sink queue (called at server startup)."""
        self._dynamo_kv_event_queue = queue
