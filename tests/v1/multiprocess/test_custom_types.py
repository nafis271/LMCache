...
def test_ipc_cache_engine_key_retention_roundtrip():
    """Retention fields survive encode/decode and stay out of cache identity."""
    ...
    assert decoded.retention_bound_tokens == 128
    ...
    assert copy.retention_bound_tokens == 128


def test_ipc_cache_engine_key_retention_wire_default():
    """A payload encoded without the retention fields decodes to 0/0."""
    payload = {
        "model_name": "test_model",
        "world_size": 4,
        "worker_id": 1,
    ...
    assert decoded.retention_bound_tokens == 0
